...
class BoosterSHOT(nn.Module):
    def __init__(self, dataset, arch='resnet18', world_feat_arch='conv',bottleneck_dim=128, outfeat_dim=64, dropout=0.5, depth_scales=4):
        ...
        proj_mats = {}
        for i in range(self.depth_scales):
            proj_mats[i] = [torch.from_numpy(Rworldgrid_from_worldcoord_mat @ worldcoord_from_imgcoord_mats[i][j])
        ...
        if world_feat_arch == 'conv':
            ...
            self.world_feat = DeformTransWorldFeat(dataset.num_cam, dataset.Rworld_shape, base_dim, n_points=n_points, stride=2, reference_points=reference_points)
        else:
            raise Exception
    ...
    def warp_perspective(self, img_feature_all, proj_mats):
        ...
        for i in range(self.depth_scales):
            ...
            out_feat = warp_perspective(feature_map, proj_mats[i], self.Rworld_shape)
    ...
    def forward(self, imgs, M, logdir=None, visualize=False):
        ...
        proj_mats = [self.proj_mats[i].repeat(B, 1, 1, 1).view(B * N, 3, 3).float() @ imgcoord_from_Rimggrid_mat.to(device)
                     for i in range(self.depth_scales)]
        ...
        world_feat = self.warp_perspective(imgs_feat, proj_mats).view(B, N, C, H, W) # {Tensor(1,6,128,160,250)}
        ##############################################
        world_feat = self.mask(world_feat)
