...
class MultiViewDynamicMask(nn.Module):
    def __init__(self,
        ...
        # ======== 新增θ参数初始化, 使用logit空间确保θ可学习且保持在[0,1]范围内 ========
        init_theta = 0.45
        self.theta_logit = nn.Parameter(torch.logit(torch.tensor(init_theta)))
    ...
    def theta(self) -> float:
        """获取当前θ值（经过sigmoid转换）"""
        with torch.no_grad():
            return torch.sigmoid(self.theta_logit).item()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # ======== 修改残差连接部分, 计算可学习θ的sigmoid值（确保在[0,1]范围内） ========
        # 门控融合: residual = θ * (masked_features) + (1 - θ) * x
        # 等价于: residual = x + θ * (masked_features - x)
        theta = torch.sigmoid(self.theta_logit)  # 可学习融合系数
        residual_features = x + theta * (weighted_features - x)
...
if __name__ == '__main__':
    ...
    output, masks = DynamicMask(test_input)  # 必须执行前向传播
    ...
    # 检查是否存在 theta 属性
    if hasattr(DynamicMask, 'theta'):
        print(f"当前theta值: {DynamicMask.theta:.4f}")
    else:
        pass