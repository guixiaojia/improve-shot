...
class fusion3(nn.Module):
    def __init__(self, num_cam, Rworld_shape, hidden_dim=128):
        ...
        self.sampler_direction_feat = 'sim'                         # ['——', 'sim']
        ################################################################### 采样组数                                产生偏移的卷积核                          余弦相似度的范围
        self.sampler0 = LocalSimGuidedSampler(in_channels=128, groups=self.sampler_groups[4], kernel_size=self.sampler_kernel_size[1], local_window=self.sampler_local_windows[3], direction_feat='sim_concat', dilation=2)
        self.sampler1 = LocalSimGuidedSampler(in_channels=128, groups=self.sampler_groups[3], kernel_size=self.sampler_kernel_size[1], local_window=self.sampler_local_windows[2], direction_feat='sim_concat', dilation=2)
    ...
    def forward(self, x, visualize=False):      # x = {tensor(1,7,128,120,360)}
        ...
        x0 = self.sampler0(for_offset, self.world_feat0(x))     # ( {tensor(1,128,120,360)}用于产生偏移 , {tensor(1,512,120,360)}用于被采样 )
        x1 = self.sampler1(for_offset, self.world_feat1(x0))    # ( {tensor(1,128,120,360)}用于产生偏移 , {tensor(1,256,120,360)}用于被采样 )
...
if __name__ == '__main__':
    ...
    sampler.to(device)
    print(f'[kernel_size={sampler.kernel_size}, groups={sampler.groups}] sim&sim_concate={sampler.direction_feat} [local_window={sampler.local_window}, dilation={sampler.dilation_for_compute_similarity}]')
