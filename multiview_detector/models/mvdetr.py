...
import 深度学习视觉框架
        |___图像预处理
        |___VGG11
import multiview_detector
        |___models
        |       |___class ResNet(nn.Module)
        |       |___class ConvWorldFeat(nn.Module)
        |       |___class DeformConvWorldFeat(nn.Module)
        |       |___class TransformerWorldFeat(nn.Module)
        |       |___class DeformTransWorldFeat(nn.Module)
        |       |___class DeformTransWorldFeat_aio(nn.Module)
        |
        |___utils
                |___class img_color_denormalize(object)
                |___def array2heatmap(heatmap)
                |___def get_worldcoord_from_imgcoord_mat(intrinsic_mat, extrinsic_mat, z=0)
                |___def project_2d_points(project_mat, input_points)
'''
...
class MVDeTr(nn.Module):
    def __init__(self, dataset, arch='resnet18', z=0, world_feat_arch='conv', bottleneck_dim=128, outfeat_dim=64, dropout=0.5):
        super().__init__()
        ...
        self.proj_mats = torch.stack([torch.from_numpy(Rworldgrid_from_worldcoord_mat @ worldcoord_from_imgcoord_mats[cam])
                                      for cam in range(dataset.num_cam)])
    ...
    def forward(self, imgs, M, logdir=None, visualize=False):
        ...
        imgcoord_from_Rimggrid_mat = inverse_affine_mats @ torch.from_numpy(np.diag([self.img_reduce, self.img_reduce, 1])).view(1, 3, 3).repeat(B * N, 1, 1).float()
        ...
        if visualize:
            ...
            proj_imgs = warp_perspective(T.Resize(self.Rimg_shape)(imgs), proj_mats.to(imgs.device), self.Rworld_shape).view(B, N, 3, self.Rworld_shape[0], self.Rworld_shape[1])
            for cam in range(N):
        ...
        world_feat = warp_perspective(imgs_feat, proj_mats.to(imgs.device), self.Rworld_shape).view(B, N, C, H, W)
        if visualize:
            for cam in range(N):
