...
class fusion3(nn.Module):
    def __init__(self, num_cam, Rworld_shape, hidden_dim=128):
        ...
        self.bottleneck = nn.Sequential(nn.Conv2d(898, hidden_dim, kernel_size=3, padding=1), nn.LeakyReLU(negative_slope=0.01), )

        # 三组编码器
        self.world_feat0 = nn.Sequential(nn.Conv2d(128, 128, 3, padding=1), nn.LeakyReLU(negative_slope=0.01))
        ...
        #############################################################
        self.sampler0 = LocalSimGuidedSampler(in_channels=128, groups=4, kernel_size=3, local_window=5, direction_feat='sim_concat', dilation=2)
        ...
        # MultiViewDynamicMask
        # from multiview_detector.models.MultiViewDynamicMask import MultiViewDynamicMask
        from multiview_detector.models.MultiViewDynamicMask_weight import MultiViewDynamicMask
