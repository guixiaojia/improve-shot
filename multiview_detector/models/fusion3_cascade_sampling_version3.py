...
class fusion3(nn.Module):
    def __init__(self, num_cam, Rworld_shape, hidden_dim=128):
        ...
        # 三组编码器
        self.world_feat0 = nn.Sequential(nn.Conv2d(898, 512, (1,1), (1,1)), nn.ReLU())
        self.world_feat1 = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, padding=1), nn.ReLU())
        self.world_feat2 = nn.Sequential(nn.Conv2d(256, hidden_dim, kernel_size=3, padding=1), nn.ReLU())
        ...
        ############################################################# 采样组数[1,2,4,8,16]                产生偏移的卷积核                             余弦相似度的范围[3,5,7,9]
        self.sampler0 = LocalSimGuidedSampler(in_channels=128, groups=self.sampler_groups[2], kernel_size=self.sampler_kernel_size[1], local_window=self.sampler_local_windows[3], direction_feat='sim_concat', dilation=2)
        self.sampler1 = LocalSimGuidedSampler(in_channels=128, groups=self.sampler_groups[2], kernel_size=self.sampler_kernel_size[1], local_window=self.sampler_local_windows[2], direction_feat='sim_concat', dilation=2)
        self.sampler2 = LocalSimGuidedSampler(in_channels=128, groups=self.sampler_groups[2], kernel_size=self.sampler_kernel_size[1], local_window=self.sampler_local_windows[1], direction_feat='sim_concat', dilation=2)
        self.sampler0_1 = LocalSimGuidedSampler(in_channels=128, groups=self.sampler_groups[1], kernel_size=self.sampler_kernel_size[1], local_window=self.sampler_local_windows[3], direction_feat='sim', dilation=1)
        self.sampler1_1 = LocalSimGuidedSampler(in_channels=128, groups=self.sampler_groups[1], kernel_size=self.sampler_kernel_size[1], local_window=self.sampler_local_windows[2], direction_feat='sim', dilation=1)
        self.sampler2_1 = LocalSimGuidedSampler(in_channels=128, groups=self.sampler_groups[1], kernel_size=self.sampler_kernel_size[1], local_window=self.sampler_local_windows[1], direction_feat='sim', dilation=1)
    ...
    def forward(self, x, visualize=False):      # x = {tensor(1,7,128,120,360)}
        for_offset = torch.mean(x, dim=1)   # for_offset = {tensor(1,128,120,360)}
        B, N, C, H, W = x.shape
        x = x.view(B, N * C, H, W)
        x = torch.cat([x, self.coord_map.repeat([B, 1, 1, 1]).to(x.device)], 1)

        x0 = self.sampler0_1(for_offset, self.sampler0(for_offset, self.world_feat0(x)))
        x1 = self.sampler1_1(for_offset, self.sampler1(for_offset, self.world_feat1(x0)))
        x2 = self.sampler2_1(for_offset, self.sampler2(for_offset, self.world_feat2(x1)))
        return x2
...
if __name__ == '__main__':
    ...
    print(sampler(torch.randn([1, 128, 120, 360]), torch.randn([1, 512, 120, 360])).shape)

    sampler = model.sampler1
    sampler.to(device)
    print(f'[kernel_size={sampler.kernel_size}, groups={sampler.groups}] {sampler.direction_feat} [local_window={sampler.local_window}, dilation={sampler.dilation_for_compute_similarity}]')
    print(sampler(torch.randn([1, 128, 120, 360]), torch.randn([1, 256, 120, 360])).shape)
