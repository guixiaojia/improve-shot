...
class fusion3(nn.Module):
    def __init__(self): # num_cam, Rworld_shape, hidden_dim=128
        ...
        self.sampler_in_channels    = 128           # 128 896
        self.sampler_groups         = 32             # [1, 2, 4, 8, 16, 32, 64, 128]
        self.sampler_local_windows  = 3             # [3, 5, 7]
        self.sampler_direction_feat = 'sim_concat'  # ['sim_concat', 'sim']
    ...
    def forward(self, for_offset, masks, x, visualize=False):      # x = {tensor(1,7,128,120,360)}
        ...
        #     ######### 分组拼接 [注意维度顺序] (1,4*128,120,360)
        #     # group0 = torch.mean(x[:, [0, 1, 2, 5], :, :, :], dim=1)
        #     # group1 = torch.mean(x[:, [3, 4, 6], :, :, :], dim=1)
        #     # group2 = x[:, 6:7, :, :, :].squeeze(1)  # 保留第6帧
        #     # for_offset = torch.cat([group0, group1], dim=1)
        #     pass
        # B, N, C, H, W = x.shape
        # x = x.view(B, N * C, H, W)
        # x = torch.cat([x, self.coord_map.repeat([B, 1, 1, 1]).to(x.device)], 1)
        # x = self.world_feat(x)
        x = self.sampler(for_offset, masks, x)     # (mean_x用于产生offset  x用于被采样的)
        return x
...
class LocalSimGuidedSampler(nn.Module):
    ...
    def forward(self, lr_x, mask, feat2sample):
        if self.direction_feat == 'sim':
        ...
        # lr_sim = self.apply_mask_to_sim(lr_sim, mask)
        offset = self.offset(lr_sim)
        result = self.sample(feat2sample, offset)
        return result

    def apply_mask_to_sim(self, lr_sim, mask):
        """
        对相似度特征进行多视角掩码操作
        参数：
            lr_sim: 低分辨率相似度特征 [B, C, H, W] (1,136,120,360)
            mask: 多视角掩码 [B, V, C_mask, H, W] (1,7,128,120,360)
        返回：
            masked_sim: 掩码后的相似度特征 [B, C, H, W]
        """

        merged_mask = mask.view(mask.size(0), -1, mask.size(3), mask.size(4))   # 1. 合并视角和通道维度以便压缩掩码维度 -> [1, 7*128=896, 120, 360]
        spatial_mask = merged_mask.max(dim=1, keepdim=True)[0]                  # 2. 沿合并后的通道维度取最大值生成最终掩码(只要有任意视角/通道在该位置为1，结果即为1) -> [1,1,120,360]
        expanded_mask = spatial_mask.expand(-1, lr_sim.size(1), -1, -1)         # 3. 扩展掩码通道维度使掩码形状与lr_sim完全一致 -> [1,136,120,360]
        masked_sim = lr_sim * expanded_mask
        return masked_sim
...
if __name__ == '__main__':
    ...
    print('local_window      =', sampler.local_window)
