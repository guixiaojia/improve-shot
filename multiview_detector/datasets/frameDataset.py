...
class frameDataset(VisionDataset):
    ...
    def __getitem__(self, index, visualize=False):
        ...
        # imgs_gt['heatmap_mask'] = kornia.warp_perspective(imgs_gt['heatmap_mask'], affine_mats, self.img_shape, align_corners=False)
        # imgs_gt['heatmap_mask'] = F.interpolate(imgs_gt['heatmap_mask'], self.Rimg_shape, mode='bilinear', align_corners=False).bool().float()
        ...
        world_gt = get_gt(self.Rworld_shape, world_pt_s[:, 0], world_pt_s[:, 1], v_s=world_pid_s, reduce=self.world_reduce, top_k=self.top_k, kernel_size=self.world_kernel_size)
...
#
#     dataset = frameDataset(Wildtrack(os.path.expanduser('/mnt/data_sdc/gjj/Data/Wildtrack')), train=True, augmentation=False)
